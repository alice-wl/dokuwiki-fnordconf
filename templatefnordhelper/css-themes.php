...
function css_out(){
    ...
    $theme  = trim(preg_replace('/[^\w-]+/','',$_REQUEST['th']));
    $tpl    = trim(preg_replace('/[^\w-]+/','',$_REQUEST['t']));

    if( !$tpl && isset( $conf['template'] )) {
        $tpl = $conf['template'];
    }
    if( !$theme && isset( $conf['template_theme'] )) {
        $theme = $conf['template_theme'];
    }
    ...
    if(isset($config_cascade['template_dir'])) {
        foreach($config_cascade['template_dir'] as $k => $v) {
            $chache.= str_replace( array( ':', '/' ), '', implode( $v ));   
        }
    }
  ...
  // load template styles
    ...
    if( $style_ini ){
        ...
        foreach($ini['themes'] as $file => $mode){
            $tplstyles['themes'][$mode][css_getpath( $tpl, $file )] = $tpldir;
        }
    }
  ...
  //if (isset($tplstyles['all'])) $files = array_merge($files, $tplstyles['all']);
    ...
    if( $theme && isset( $tplstyles['themes'][$theme] )) {
        $files = array_merge($files, $tplstyles['themes'][$theme]);
    } elseif( $theme ) {
        $theme = false;
    }
  ...
  // The generated script depends on some dynamic options
    $cache = getCacheName('styles'.$_SERVER['HTTP_HOST'].$_SERVER['SERVER_PORT'].DOKU_BASE.$cache.$style.$tpl.$theme,'.css');

  // check cache age & handle conditional request
    header('Cache-Control: public, max-age=3600');
    header('Pragma: public');
    if(css_cacheok($cache,array_merge( array_keys($files)), css_getpath( $tpl, 'style.ini' ))){
    ...
    $css = css_applystyle($css,$tpl,$theme);
 ...
 * @author Andreas Gohr <andi@splitbrain.org>
 *
 *  replaced tplinc with styleini 
 */
function css_cacheok($cache,$files,$style_ini){
 ...
 *  replaced tplinc with styleini 
 */
function css_applystyle($css,$tpl,$theme=false){
    global $conf;
    if( $file = css_getpath( $tpl, 'style.ini' )) {
      $ini = parse_ini_file( $file, true);
    // parse replacments 
      if( isset( $ini['replacements-'.$theme] )) {
          foreach( $ini['replacements-'.$theme] as $k => $v ) {
              $ini['replacements'][$k] = $v;
          }
      }
...
$fp = fopen(DOKU_INC.'csspath.log', 'a');
fwrite( $fp, DOKU_INC.'lib/tpl/'.$t.'/'.$file."\n" );
fclose($fp);
   return DOKU_INC.'lib/tpl/'.$t.'/'.$file;
}
